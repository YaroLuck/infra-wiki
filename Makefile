SHELL := /bin/bash
SERVICES := reverse-proxy docmost bookstack
NET := web
BACKUP_DIR := backups
...
help:
        ...
        @echo "  up dir=<svc>     - поднять сервис (reverse-proxy|docmost|bookstack)"
...
define need_dir
        @if [ -z "$(dir)" ]; then echo "Укажи: make <target> dir=docmost|reverse-proxy|bookstack"; exit 1; fi
endef
